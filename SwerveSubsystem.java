...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  public SwerveSubsystem(File directory)
  {
    ...
    double angleConversionFactor = SwerveMath.calculateDegreesPerSteeringRotation(14.4);
    // Motor conversion factor is (PI * WHEEL DIAMETER IN METERS) / (GEAR RATIO * ENCODER RESOLUTION).
    //  In this case the wheel diameter is 4 inches, which must be converted to meters to get meters/second.
    //  The gear ratio is 6.75 motor revolutions per wheel rotation.
    //  The encoder resolution per motor revolution is 1 per motor revolution.
    double driveConversionFactor = SwerveMath.calculateMetersPerRotation(Units.inchesToMeters(3.78), 6.12);
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public Command driveCommand(DoubleSupplier translationX, DoubleSupplier translationY, DoubleSupplier angularRotationX)
  {
    return run(() -> {
      // Make the robot move
      swerveDrive.drive(new Translation2d(powerof2(translationX.getAsDouble()) * .95 * swerveDrive.getMaximumChassisVelocity(),
                            powerof2(translationY.getAsDouble()) * .95 * swerveDrive.getMaximumChassisVelocity()),
                            powerof2(angularRotationX.getAsDouble()) * swerveDrive.getMaximumChassisAngularVelocity(),
                        true,
                        false);
    });
  }
